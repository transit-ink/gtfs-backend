...
const batchSize = batchSizeArg ? parseInt(batchSizeArg.split('=')[1], 10) : 1000; // default batch size

if (!gtfsPath) {
  console.error(
    'Usage: node import-gtfs.js <path-to-gtfs-files> [--clean] [--batch-size=<number>]'
  );
...
function buildInsertQuery(tableName, columns, values) {
  ...
  const valueRows = values.map(row => `(${row.map(escapeValue).join(', ')})`).join(',\n       '); // prettier spacing if you're logging it
...
async function importGTFSFile(filePath, tableName) {
  ...
  try {
    ...
    for (let i = 0; i < totalRecords; i += batchSize) {
      ...
      const values = batch.map(record => columns.map(col => record[col] || null));
